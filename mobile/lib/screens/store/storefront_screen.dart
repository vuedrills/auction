  ...
  void initState() {
    ...
    Future.microtask(() async {
      try {
        ...
        ref.read(analyticsServiceProvider).trackEvent(storeId: store.id, eventType: 'store_view');
      } catch (_) {}
    });
  }
    ...
    return Scaffold(
      ...
      body: storeAsync.when(
        ...
        data: (store) {
                          ...
                          children: [
                            ...
                            if (store.phone != null) ...[
                              InkWell(
                                onTap: () {
                                  // Track click
                                  ref.read(analyticsServiceProvider).trackEvent(storeId: store.id, eventType: 'call_click');
                            ...
                            if (store.whatsapp != null) ...[
                              InkWell(
                                onTap: () {
                                  // Track click
                                  ref.read(analyticsServiceProvider).trackEvent(storeId: store.id, eventType: 'whatsapp_click');
          ...
          // Apply store-level desaturation if store is stale (all products are stale)
          // Use ClipRect + RepaintBoundary to prevent grey bleeding onto scaffold
          if (store.isStale) {
            content = ClipRect(
...
class _ProductCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isProductStale = daysSinceUpdate > 30;
    ...
    if (isProductStale) {
      // Wrap with ClipRRect and RepaintBoundary to prevent color bleeding
